defmodule SwaiWeb.CoreComponents do
  @moduledoc """
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-zinc-900 hover:bg-zinc-700 py-2 px-3 mt-2",
  ...
  def error(assigns) do
    ~H"""
    <p class="flex gap-1 text-sm leading-6 text-rose-600 phx-no-feedback:hidden">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SwaiWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SwaiWeb.Gettext, "errors", msg, opts)
    end
  end
