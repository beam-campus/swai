defmodule TradeWarsWeb.CoreComponents do
  @moduledoc """
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg leading-8 font-brand font-regular text-ltOrange-light bg-ltDark">
          <%= render_slot(@inner_block) %>
        </h1>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TradeWarsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TradeWarsWeb.Gettext, "errors", msg, opts)
    end
  end
