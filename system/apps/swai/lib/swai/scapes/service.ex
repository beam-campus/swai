defmodule Scapes.Service do
  ...
  def handle_info({@initializing_scape_v1, scape_init}, state) do
    :scapes_cache
    |> Cachex.put!(scape_init.id, scape_init)
  ...
  ####################### INITIALIZED SCAPE #######################
  @impl true
  def handle_info({@scape_initialized_v1, scape_init}, state) do
    :scapes_cache
    |> Cachex.put!(scape_init.id, scape_init)
  ...
  ####################### DETACHED SCAPE #######################
  @impl true
  def handle_info({@scape_detached_v1, scape_init}, state) do
    :scapes_cache
    |> Cachex.del!(scape_init.id)
  ...
  ########################### SCAPE STARTED #######################
  @impl true
  def handle_info({@scape_started_v1, scape_init}, state) do
    scape =
      case :scapes_cache
           |> Cachex.get!(scape_init.id) do
        {:ok, nil} ->
          scape_init
    ...
    scape =
      %ScapeInit{
        scape
        | status: @scape_started_status
      }

    case :scapes_cache
    |> Cachex.put!(scape.id, scape) do
      {:ok, _} ->
